 ...
 */
public class WebSocketFriendUtil {
    ...
    //饿汉式
    private static WebSocketFriendUtil instance = new WebSocketFriendUtil();

    private WebSocketFriendUtil() {
    }

    public static WebSocketFriendUtil getInstance() {
        return instance;
    }
    ...
    public void init(Context context,String name, MessageCallBack messageCallBack) {
        ...
        try {
            wsManager = new WsManager.Builder(context)
                    ...
                    .wsUrl(Properties.wsUrlFriend+name)
                    .needReconnect(true)
                    .build();
            wsManager.startConnect();
            wsManager.setWsStatusListener(wsStatusListener);
        } catch (Exception e) {
            Log.e(TAG, "初始化异常(好友请求)-----------" + e.getMessage());
        }
    }
    ...
    public void sendDataD(String content) {
        ...
        if (wsManager != null && wsManager.isWsConnected()) {
            ...
            if (isSend) {
                Log.e("onOpen sendMessage", "发送成功(好友请求)");
            } else {
                Log.e("onOpen sendMessage", "发送失败(好友请求)");
            }
        } else {
            Toast.makeText(context, "请先连接服务器(好友请求)", Toast.LENGTH_SHORT).show();
        }
